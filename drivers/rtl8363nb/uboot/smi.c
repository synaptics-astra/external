						...
						rtk_uint32 data)
{
	...
	ret = miiphy_write(	devname, (unsigned char)phy_id,
						(unsigned char)register_id, (unsigned short)data);

	if (ret)
		printf("%s: got error code %d from miiphy_write\n", __func__, ret);
}
					...
					rtk_uint32 *pData)
{
	...
	ret = miiphy_read(	devname, (unsigned char)phy_id,
						(unsigned char)register_id, &value);

	*pData = (rtk_uint32)value;

	if (ret)
		printf("%s: got error code %d from miiphy_read\n", __func__, ret);
}
